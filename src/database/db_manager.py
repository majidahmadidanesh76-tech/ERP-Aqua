...
class DatabaseManager:
    ...
    def start_production_cycle(self, cage_id, start_date, species, initial_count, 
        ...
        # اول همه دوره‌های قبلی را غیرفعال کن
    ...
    def get_feeds_by_cage_and_cycle(self, cage_id, cycle_id):
        return self.fetch_all(
            "SELECT * FROM feeds WHERE cage_id = %s AND cycle_id = %s ORDER BY date",
            (cage_id, cycle_id)
        )
    ...
    def get_mortalities_by_cage_and_cycle(self, cage_id, cycle_id):
        return self.fetch_all(
            "SELECT * FROM mortalities WHERE cage_id = %s AND cycle_id = %s ORDER BY date",
            (cage_id, cycle_id)
        )
    ...
    def get_biomasses_by_cage_and_cycle(self, cage_id, cycle_id):
        return self.fetch_all(
            "SELECT * FROM biomasses WHERE cage_id = %s AND cycle_id = %s ORDER BY date",
            (cage_id, cycle_id)
        )
