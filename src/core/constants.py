...
# سطوح دسترسی سازمانی
ACCESS_LEVELS = {
    "employee": 1,      # کارمند عادی (فقط کارتابل خودش)
    "supervisor": 2,    # سرپرست (کارتابل خودش + زیردستان مستقیم)
    "manager": 3,       # مدیر (کارتابل خودش + زیردستان + واحد زیرمجموعه)
    "vp": 4,            # معاون (دسترسی به کل معاونت)
    "ceo": 5,           # مدیرعامل (دسترسی به کل سازمان)
    "admin": 6          # ادمین (دسترسی کامل)
}