"""
صفحه برنامه‌ریزی تولید - شامل تقویم رشد و برنامه شیفتی
نسخه کامل با برنامه شیفتی
"""
...
class ProductionPlanningTab(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        title = QtWidgets.QLabel("📋 برنامه‌ریزی تولید")
        ...
        self.tabs.addTab(self.budget_tab, "📊 بودجه‌بندی سالانه")
    ...
    def create_growth_calendar_tab(self):
        ...
        saved_group = QtWidgets.QGroupBox("📋 برنامه‌های ذخیره شده")
        ...
        self.calc_btn = QtWidgets.QPushButton("🔮 محاسبه پیش‌بینی رشد")
        ...
        self.result_group = QtWidgets.QGroupBox("📊 نتایج پیش‌بینی")
        ...
        fcr_lbl = QtWidgets.QLabel("📊 ضریب تبدیل پیش‌بینی:")
            ...
            if plan:
                ...
                for i in range(self.species_combo.count()):
                    if self.species_combo.itemData(i) == species_id:
                        ...
                        break
                date_parts = plan['planned_stocking_date'].split('-')
    ...
    def save_current_plan(self):
        ...
        if not hasattr(self, 'current_forecast'):
            QtWidgets.QMessageBox.warning(self, "خطا", "لطفاً ابتدا پیش‌بینی را محاسبه کنید")
            return
    ...
    def create_shift_planning_tab(self):
        ...
        self.refresh_tasks_btn = QtWidgets.QPushButton("🔄 به‌روزرسانی")
    ...
    def load_daily_tasks(self):
        ...
        print(f"DEBUG: بارگذاری وظایف برای تاریخ {task_date}")
        
        tasks = self.db.get_daily_tasks_by_date(task_date)
        print(f"DEBUG: تعداد وظایف دریافت شده: {len(tasks)}")
        ...
        for i, task in enumerate(tasks):
            ...
            if task['status'] != 'done':
                ...
                complete_btn.setIconSize(QtCore.QSize(18, 18))
            ...
            delete_btn.setIconSize(QtCore.QSize(18, 18))
    ...
    def add_new_task(self):
        ...
        if dialog.exec_():
            ...
            print(f"DEBUG: ذخیره وظیفه جدید - تاریخ: {result['date']}, نوع: {result['task_type']}, مسئول: {result['assigned_to']}")
            
            success = self.db.save_daily_task(
                plan_id=0,
            ...
            if success:
                print("DEBUG: وظیفه با موفقیت ذخیره شد")
                self.load_daily_tasks()
                QtWidgets.QMessageBox.information(self, "موفق", "وظیفه با موفقیت اضافه شد")
            else:
                print("DEBUG: خطا در ذخیره وظیفه")
                QtWidgets.QMessageBox.warning(self, "خطا", "خطا در ذخیره وظیفه")

    def edit_task(self, task_id):
        """ویرایش وظیفه"""
        tasks = self.db.get_daily_tasks_by_date(self.shift_date.date().toString("yyyy-MM-dd"))
        task = None
        for t in tasks:
            if t['id'] == task_id:
                task = t
        ...
        dialog = TaskDialog(self, task['task_date'], task)
        if dialog.exec_():
            result = dialog.get_data()
    ...
    def create_budget_tab(self):
        tab = QtWidgets.QWidget()
        layout = QtWidgets.QVBoxLayout(tab)
        info_label = QtWidgets.QLabel("📊 بودجه‌بندی سالانه - در حال توسعه")
        info_label.setAlignment(QtCore.Qt.AlignCenter)
        info_label.setStyleSheet("font-size: 14px; color: #C8C8C8; padding: 50px;")
...
class TaskDialog(QtWidgets.QDialog):
    ...
    def setup_ui(self):
        ...
        self.task_type.addItems(["تغذیه", "نمونه‌برداری", "بازرسی تور", "ثبت تلفات", "نگهداری", "تعمیرات", "برداشت", "سایر"])
